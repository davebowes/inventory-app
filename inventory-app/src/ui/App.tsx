...
type SettingsTab = "products" | "locations" | "materialTypes" | "import";
  ...
  return (
    ...
    >
      {locations.length === 0 ? (
        ...
        <>
          <div className="hint">
            Enter quantities with <b>one decimal place</b> (example: 12.3). Totals are summed across locations on the
            Reorder tab.
          </div>

          {visibleProducts.length === 0 ? (
            <div className="hint" style={{ marginTop: 8 }}>
              No items are assigned to <b>{location?.name ?? "this location"}</b>. Assign products to this location in{" "}
              <b>Settings → Products</b>.
            </div>
          ) : (
            <>
              {/* Desktop table */}
              ...
              {/* Mobile compact row list (no horizontal scroll) */}
...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  function buildEmailBody() {
    ...
    for (const [groupName, list] of groups) {
      ...
      for (const r of list) lines.push(`- ${r.name} (${r.sku}): ${String(r.to_order)}`);
      lines.push("");
    }
    return lines.join("\n").trim();
  }
    ...
    >
      ...
      <div className="printOnly">
        ...
        <div style={{ marginTop: 12, display: "grid", gap: 14 }}>
          {groups.map(([groupName, list]) => (
            <div key={groupName}>
              <div style={{ fontWeight: 800, marginBottom: 6 }}>{groupName}</div>
...
function ImportSettings({ onError }: { onError: (m: string) => void }) {
  ...
  function mapRow(rec: Record<string, string>) {
    ...
    return { location: location || undefined, material_type: material_type || undefined, sku, name, par: par || undefined, on_hand: on_hand || undefined };
  }

  return (
    <Card
      title="Import CSV"
      right={<div className="muted">Tip: you can paste CSV below or upload a .csv file.</div>}
    >
      <div className="stack">
        <div className="muted">
          <div>
            <b>Required columns:</b> sku, name
          </div>
          <div>
            <b>Optional:</b> material_type, par, location, on_hand
          </div>
          <div className="mutedSmall" style={{ marginTop: 6 }}>
            If you include <b>location</b> + <b>on_hand</b>, it will also set on‑hand quantities per location. You can include multiple rows for the same SKU across different locations.
          </div>
        </div>
            ...
            onClick={() => {
              void (async () => {
                try {
                  ...
                  const rows = recs.map(mapRow).filter((r) => (r.sku ?? "").trim() && (r.name ?? "").trim());
                  ...
                  const result = await Api.importRows(rows as any);
...
SKU-001,1/2 MDF,Sheet Goods,10.5,Shop,2.0`}
        ...
        {lastResult && (
          <div className="cardSub">
            <b>Import complete.</b>
            <div className="mutedSmall" style={{ marginTop: 6 }}>
              Rows received: {lastResult.rows_received} • Rows imported: {lastResult.rows_imported} • Locations seen:{" "}
              {lastResult.locations_seen} • Material Types seen: {lastResult.material_types_seen} • On‑hand rows updated:{" "}
              {lastResult.on_hand_upserts}
            </div>
          </div>
        )}
      </div>
...
function ProductsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  async function load() {
    try {
      const [prods, mts, locs] = await Promise.all([Api.listProducts(), Api.listMaterialTypes(), Api.listLocations()]);
      setProducts(prods);
  ...
  async function submit() {
    ...
    try {
      if (editing) {
        await Api.updateProduct(editing.id, { name: n, sku: s, material_type_id: mtId, par: p, location_ids: selectedLocationIds });
      } else {
        await Api.createProduct({ name: n, sku: s, material_type_id: mtId, par: p, location_ids: selectedLocationIds });
      }
      resetForm();
      await load();
    } catch (e: any) {
    ...
    <div>
      ...
      <div className="row" style={{ justifyContent: "flex-end", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
        <button
          ...
          type="button"
          onClick={() => {
            if (!confirm("Clear ALL products? This will also remove assigned locations and on-hand entries.")) return;
            void (async () => {
              try {
...
const styles = `
  :root { --red: #E31837; --border: #e5e7eb; --text: #111827; --muted: #6b7280; --bg: #ffffff; }
  * { box-sizing: border-box; }
  ...
  .appHeader { display: flex; align-items: flex-end; justify-content: space-between; gap: 12px; margin: 6px 0 10px; }
  .appTitle { margin: 0; color: var(--red); font-size: 26px; letter-spacing: -0.2px; }
  .topNav { display: flex; gap: 8px; flex-wrap: wrap; }

  .topNavBtn { padding: 8px 12px; border-radius: 999px; border: 1px solid #d1d5db; background: #ffffff; color: var(--text); cursor: pointer; }
  .topNavBtnActive { border: 1px solid #111827; background: #111827; color: #ffffff; }

  .errorBox { background: #fee2e2; border: 1px solid #fecaca; padding: 12px; border-radius: 12px; margin: 10px 0 12px; }
  .appBody { margin-top: 8px; }

  .card { border: 1px solid var(--border); border-radius: 16px; padding: 14px; box-shadow: 0 1px 2px rgba(0,0,0,0.05); margin-bottom: 14px; background: var(--bg); }
  .cardHeader { display: flex; justify-content: space-between; gap: 10px; align-items: center; margin-bottom: 10px; flex-wrap: wrap; }
  ...
  .input, .select { padding: 10px 12px; border-radius: 12px; border: 1px solid #d1d5db; min-width: 180px; font-size: 14px; }
  .select { background: #ffffff; }

  .btn { padding: 10px 12px; border-radius: 12px; border: 1px solid #111827; background: #111827; color: #ffffff; cursor: pointer; }
  .btn.danger { border-color: var(--red); background: var(--red); color: #ffffff; }
  .btn.danger:disabled { opacity: 0.7; cursor: not-allowed; }

  .btnSecondary { padding: 10px 12px; border-radius: 12px; border: 1px solid #d1d5db; background: #ffffff; color: #111827; cursor: pointer; }
  .btnDanger { padding: 10px 12px; border-radius: 12px; border: 1px solid #ef4444; background: #ef4444; color: #ffffff; cursor: pointer; }

  .linkBtn { border: none; background: transparent; color: #ef4444; cursor: pointer; padding: 0; text-decoration: underline; font-size: 12px; }

  .tableWrap { overflow: hidden; border: 1px solid var(--border); border-radius: 14px; }
  .table { width: 100%; border-collapse: collapse; }
  .th { text-align: left; font-size: 12px; color: #374151; padding: 10px 12px; border-bottom: 1px solid var(--border); background: #f9fafb; white-space: nowrap; }
  .td { padding: 10px 12px; border-bottom: 1px solid var(--border); vertical-align: middle; font-size: 14px; }
  .td.strong { font-weight: 800; }

  .qtyInput { width: 130px; padding: 8px 10px; border-radius: 12px; border: 1px solid #d1d5db; font-size: 14px; }
  ...
  .chip { display: flex; align-items: center; gap: 6px; padding: 8px 10px; border: 1px solid var(--border); border-radius: 999px; background: #f9fafb; cursor: pointer; user-select: none; }
  .chip input { width: 16px; height: 16px; }
  .chipOn { background: #e0f2fe; border-color: #7dd3fc; }

  .pCard { border: 1px solid var(--border); border-radius: 16px; padding: 12px; background: #ffffff; box-shadow: 0 1px 0 rgba(0,0,0,0.03); display: grid; gap: 6px; margin-bottom: 10px; }
  ...
  .mobileReorderRow { display: flex; justify-content: space-between; align-items: center; gap: 10px; padding: 8px 10px; border: 1px solid var(--border); border-radius: 14px; background: #ffffff; }
  .mobileRowLeft { min-width: 0; }
  .mobileRowTitle { font-weight: 800; line-height: 1.15; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 62vw; }
  .mobileRowSku { font-size: 12px; color: var(--muted); margin-top: 2px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 62vw; }
  .mobileOrderPill { font-weight: 900; font-size: 16px; padding: 10px 12px; border-radius: 999px; background: #fef3c7; border: 1px solid #f59e0b; min-width: 72px; text-align: center; }

  .settingsCredit { margin-top: 14px; padding-top: 10px; border-top: 1px solid var(--border); color: var(--muted); font-size: 12px; text-align: center; }

  .mobileRowHeader { display: grid; grid-template-columns: 1fr 140px; gap: 10px; padding: 10px 8px; color: #374151; font-size: 12px; border-bottom: 1px solid var(--border); }
  .mobileRows { display: grid; }
  .mobileRow { display: grid; grid-template-columns: 1fr 140px; gap: 10px; padding: 12px 8px; border-bottom: 1px solid var(--border); }
  .mobileRowRight { display: grid; gap: 8px; align-items: center; justify-content: end; }
  .qtyInputMobileBig { width: 96px; padding: 10px 10px; border-radius: 10px; border: 1px solid #d1d5db; font-size: 18px; text-align: right; }
  .mobileRowSub { margin-top: 2px; font-size: 12px; color: #6b7280; line-height: 1.1; }
  ...
  .bottomNavBtn { padding: 12px 10px; border-radius: 14px; border: 1px solid #d1d5db; background: #ffffff; font-weight: 800; cursor: pointer; }
  ...
  @media print {
    ...
    .topNav, .hint, .btn, .input, .settingsCredit, .bottomNav { display: none !important; }
