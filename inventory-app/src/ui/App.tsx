...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  type VendorGroup = { vendor: string; materials: Array<[string, ReorderRow[]]> };
  ...
  const groups = useMemo((): VendorGroup[] => {
    ...
    return vendors.map(([vendor, matMap]) => {
      const materials = Array.from(matMap.entries()).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
      for (const [, list] of materials) list.sort((a, b) => b.to_order - a.to_order);
  ...
  function buildEmailBody() {
    ...
    for (const vg of groups) {
      ...
      for (const [groupName, list] of vg.materials) {
          ...
          {groups.map((vg) => (
            <div key={vg.vendor} className="vendorBlock">
              ...
              <div className="groupStack">
                {vg.materials.map(([groupName, list]) => (
