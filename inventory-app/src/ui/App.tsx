    ...
    >
      <div className="stack">
        <div className="muted">
          ...
          <div><b>Optional:</b> material_type, vendor, par, location, on_hand</div>
...
function ProductsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  async function submit() {
    ...
    const p = Math.max(0, round1(pNum));
    const mtId = material_type_id === "" ? null : material_type_id;
    const vId = vendor_id === "" ? null : vendor_id;

    // PAR is always required (prevents accidentally overwriting with 0.0).
    if (!par.trim() || !Number.isFinite(pNum)) return onError("PAR is required");

    // For NEW products, require every input to be completed.
    if (!editing) {
      if (!n) return onError("Product name is required");
      if (!s) return onError("SKU is required");
      if (material_type_id === "") return onError("Material type is required");
      if (vendor_id === "") return onError("Vendor is required");
    }
