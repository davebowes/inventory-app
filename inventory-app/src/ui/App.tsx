...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  const groups = useMemo(() => {
    // vendor -> material type -> rows
    const vendorMap = new Map<string, Map<string, ReorderRow[]>>();
    for (const r of rows) {
      const vKey = (r.vendor_name ?? "No Vendor").trim() || "No Vendor";
      const mKey = (r.material_type_name ?? "Uncategorized").trim() || "Uncategorized";
      if (!vendorMap.has(vKey)) vendorMap.set(vKey, new Map());
      const mtMap = vendorMap.get(vKey)!;
      if (!mtMap.has(mKey)) mtMap.set(mKey, []);
      mtMap.get(mKey)!.push(r);
    }

    const vendors = Array.from(vendorMap.entries()).sort((a, b) =>
      a[0].toLowerCase().localeCompare(b[0].toLowerCase())
    );

    return vendors.map(([vendorName, mtMap]) => {
      const mts = Array.from(mtMap.entries()).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
      for (const [, list] of mts) list.sort((a, b) => b.to_order - a.to_order || a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
      return { vendorName, groups: mts };
    });
  }, [rows]);
  ...
  function buildEmailBody() {
    ...
    for (const v of groups) {
      ...
      for (const [groupName, list] of v.groups) {
