...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  async function load() {
    ...
    try {
      ...
      // API should already filter to_order > 0, but keep a defensive filter.
  ...
  // Group: Vendor -> Material Type -> Rows
  const vendorGroups = useMemo(() => {
    const vendorMap = new Map<string, Map<string, ReorderRow[]>>();

    for (const r of rows) {
      const vendorName = (r.vendor_name ?? "No Vendor").trim() || "No Vendor";
      const mtName = (r.material_type_name ?? "Uncategorized").trim() || "Uncategorized";
      if (!vendorMap.has(vendorName)) vendorMap.set(vendorName, new Map());
      const mtMap = vendorMap.get(vendorName)!;
      if (!mtMap.has(mtName)) mtMap.set(mtName, []);
      mtMap.get(mtName)!.push(r);
    }

    const vendors = Array.from(vendorMap.entries()).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
    return vendors.map(([vendorName, mtMap]) => {
      ...
      for (const [, list] of mts) {
        list.sort((a, b) => b.to_order - a.to_order || a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
      }
  ...
  function buildEmailBody() {
    ...
    for (const v of vendorGroups) {
      ...
      for (const [mtName, list] of v.groups) {
        lines.push(`  ${mtName}`);
        for (const r of list) {
  ...
  return (
    ...
    >
      <div className="hint">Only items that need ordering are shown, grouped by vendor then material category.</div>
