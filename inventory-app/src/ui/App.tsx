  ...
  return (
    <div className="appShell">
      ...
      <div className="appHeader">
        <div className="brandBlock" role="banner" aria-label="FASTSIGNS Inventory">
          <img className="brandLogo" src="/fastsigns-logo.png" alt="FASTSIGNS" />
      ...
      <div className="mobileOnly bottomNav" role="navigation" aria-label="Primary">
        ...
        <BottomNavButton active={view === "reorder"} color="red" onClick={() => setView("reorder")}>
          Reorder
        </BottomNavButton>
...
function OnHand({ onError }: { onError: (m: string) => void }) {
  ...
  async function save(product_id: number) {
    ...
    try {
      ...
      await Api.upsertOnHand({ product_id, location_id: locationId, qty: clampMax(clampNonNeg(num)) });
      setQtyByProductId((m) => ({ ...m, [product_id]: fmt1(clampMax(clampNonNeg(num))) }));
    } catch (e: any) {
  ...
  async function saveWithValue(product_id: number, nextValue: string) {
    // Used by +/- buttons so we don't race React state updates.
    if (locationId === "") return;
    setSavingId(product_id);
    try {
      const num = round1(Number(nextValue));
      await Api.upsertOnHand({ product_id, location_id: locationId, qty: clampMax(clampNonNeg(num)) });
      setQtyByProductId((m) => ({ ...m, [product_id]: fmt1(clampMax(clampNonNeg(num))) }));
    } catch (e: any) {
      onError(e?.message || "Failed to save on‑hand");
...
<input
                    ...
                    <button
                      ...
                      aria-label="Increase"
                      onClick={() => {
                        const cur = Number(qtyByProductId[p.id] ?? "0");
                        const safeCur = Number.isFinite(cur) ? cur : 0;
                        const next = fmt1(clampMax(round1(safeCur + 1)));
                        setQtyByProductId((m) => ({ ...m, [p.id]: next }));
                        void saveWithValue(p.id, next);
                      }}
                    >
                      +
                    </button>
...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  // Vendor headers are rendered above each vendor section (non-sticky) so they don't
  // cover content on small screens.
  ...
  // (Previously we used sticky vendor headers and dynamically calculated offsets.
  // Removing that behavior keeps the first rows from being hidden.)
...
function Settings({ onError }: { onError: (m: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="settingsCredit">
        © {new Date().getFullYear()} FASTSIGNS® • Internal Inventory Management System
        <br />
        Built by Dave Bowes
      </div>
    </Card>
...
function LocationsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  async function del(id: number) {
    const usedBy = products.filter((p) => (p.location_ids ?? []).includes(id));
    if (usedBy.length) {
      onError(`Cannot delete this location because it is assigned to ${usedBy.length} product(s). Remove it from those products first.`);
      return;
    }
        ...
        {items.map((l) => (
          <div key={l.id} className="listRow">
            ...
            <button
              ...
              className="btnDanger listRowAction"
              type="button"
              disabled={products.some((p) => (p.location_ids ?? []).includes(l.id))}
              title={
                products.some((p) => (p.location_ids ?? []).includes(l.id))
                  ? "This location is assigned to one or more products"
                  : ""
              }
            >
              Delete
...
function MaterialTypesSettings({ onError }: { onError: (m: string) => void }) {
  ...
  async function load() {
    try {
      const [mts, prods] = await Promise.all([Api.listMaterialTypes(), Api.listProducts()]);
      setItems(mts);
      setProducts(prods);
    } catch (e: any) {
  ...
  async function del(id: number) {
    const usedBy = products.filter((p) => (p.material_type_id ?? -1) === id);
    if (usedBy.length) {
      onError(
        `Cannot delete this material type because it is assigned to ${usedBy.length} product(s). Move those products to another material type first.`
      );
      return;
    }
      ...
      <div className="list">
        {items.map((mt) => (
          <div key={mt.id} className="listRow">
            ...
            <button
              onClick={() => void del(mt.id)}
              className="btnDanger listRowAction"
              type="button"
              disabled={products.some((p) => (p.material_type_id ?? -1) === mt.id)}
              title={
                products.some((p) => (p.material_type_id ?? -1) === mt.id)
                  ? "This material type is assigned to one or more products"
                  : ""
              }
            >
              Delete
...
function VendorsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  async function load() {
    try {
      const [v, prods] = await Promise.all([Api.listVendors(), Api.listProducts()]);
  ...
  async function del(id: number) {
    const usedBy = products.filter((p) => (p.vendor_id ?? -1) === id);
    if (usedBy.length) {
      onError(`Cannot delete this vendor because it is assigned to ${usedBy.length} product(s). Reassign those products first.`);
      return;
    }
        ...
        {items.map((v) => (
          <div key={v.id} className="listRow">
            ...
            <button
              ...
              disabled={products.some((p) => (p.vendor_id ?? -1) === v.id)}
              title={
                products.some((p) => (p.vendor_id ?? -1) === v.id)
                  ? "This vendor is assigned to one or more products"
                  : ""
              }
            >
              Delete
...
function ProductsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  const [filterMaterialTypeId, setFilterMaterialTypeId] = useState<number | "all">("all");
  const [sortKey, setSortKey] = useState<
    "name_az" | "sku_az" | "vendor_az" | "material_az" | "par_desc" | "par_asc"
  >("name_az");
  ...
  const [fieldErrors, setFieldErrors] = useState<
    Partial<Record<"sku" | "name" | "material" | "vendor" | "par" | "locations", string>>
  >({});
  ...
  async function load() {
    try {
      const [prods, mts, locs, vends] = await Promise.all([
        Api.listProducts(),
        Api.listMaterialTypes(),
        Api.listLocations(),
        Api.listVendors(),
      ]);

      // Normalize (API sometimes includes *_name fields)
      setProducts(prods);
      ...
      // Ensure sensible default selection when adding new products
  ...
  const materialNameById = useMemo(() => {
    const m = new Map<number, string>();
    for (const mt of materialTypes) m.set(mt.id, mt.name);
    return m;
  }, [materialTypes]);
  ...
  const filteredProducts = useMemo(() => {
    ...
    let rows = products.filter((p) => {
      if (filterVendorId !== "all" && (p.vendor_id ?? -1) !== filterVendorId) return false;
      if (filterMaterialTypeId !== "all" && (p.material_type_id ?? -1) !== filterMaterialTypeId) return false;

      if (!needle) return true;

      const skuStr = (p.sku ?? "").toString().toLowerCase();
      const nameStr = (p.name ?? "").toString().toLowerCase();
      const vendorStr = (p.vendor_name ?? vendorNameById.get(p.vendor_id ?? -1) ?? "").toLowerCase();
      const matStr = (p.material_type_name ?? materialNameById.get(p.material_type_id ?? -1) ?? "").toLowerCase();

      return skuStr.includes(needle) || nameStr.includes(needle) || vendorStr.includes(needle) || matStr.includes(needle);
    });

    const getVendor = (p: Product) => (p.vendor_name ?? vendorNameById.get(p.vendor_id ?? -1) ?? "").toLowerCase();
    const getMat = (p: Product) => (p.material_type_name ?? materialNameById.get(p.material_type_id ?? -1) ?? "").toLowerCase();
    ...
    rows = [...rows].sort((a, b) => {
      switch (sortKey) {
        ...
        case "vendor_az": {
          const d = getVendor(a).localeCompare(getVendor(b));
          if (d !== 0) return d;
          return (a.name ?? "").toLowerCase().localeCompare((b.name ?? "").toLowerCase());
        }
        case "material_az": {
          const d = getMat(a).localeCompare(getMat(b));
          if (d !== 0) return d;
          return (a.name ?? "").toLowerCase().localeCompare((b.name ?? "").toLowerCase());
        }
        case "par_desc":
          return (b.par ?? 0) - (a.par ?? 0);
        case "par_asc":
          return (a.par ?? 0) - (b.par ?? 0);
        default:
          return 0;
      }
    });
    ...
    return rows;
  }, [products, q, filterVendorId, filterMaterialTypeId, sortKey, vendorNameById, materialNameById]);
  ...
  async function submitModal() {
    ...
    if (!s) errs.sku = "SKU is required";
      ...
      const dup = products.some((pp) => (editing ? pp.id !== editing.id : true) && (pp.sku ?? "").toUpperCase() === s);
  ...
  return (
    <div>
      <div className="hint">
        Products carry a <b>global PAR</b>. On‑hand is tracked per location.
      </div>

      {/* Toolbar: search / filter / sort / add */}
      <div className="pToolbar noPrint">
        <input
          value={q}
        ...
        <select
          className="select"
          value={filterVendorId === "all" ? "all" : String(filterVendorId)}
        ...
        <select
          className="select"
          value={filterMaterialTypeId === "all" ? "all" : String(filterMaterialTypeId)}
          onChange={(e) => setFilterMaterialTypeId(e.target.value === "all" ? "all" : Number(e.target.value))}
        >
        ...
        <select className="select" value={sortKey} onChange={(e) => setSortKey(e.target.value as any)}>
          ...
          <option value="sku_az">Sort: SKU A–Z</option>
          <option value="vendor_az">Sort: Vendor A–Z</option>
          <option value="material_az">Sort: Material A–Z</option>
          <option value="par_desc">Sort: PAR high → low</option>
          <option value="par_asc">Sort: PAR low → high</option>
        </select>
        ...
        <table className="table">
          <thead>
            <tr>
              ...
              <th className="th" style={{ textAlign: "right" }}>PAR</th>
          ...
          <tbody>
            {filteredProducts.map((p) => (
              <tr key={p.id}>
                <td className="td">{p.sku}</td>
                <td className="td">{p.name}</td>
                <td className="td">{p.vendor_name ?? vendorNameById.get(p.vendor_id ?? -1) ?? ""}</td>
                <td className="td">{p.material_type_name ?? materialNameById.get(p.material_type_id ?? -1) ?? "Uncategorized"}</td>
                <td className="td" style={{ textAlign: "right" }}>{fmt1(p.par)}</td>
                <td className="td" style={{ textAlign: "right" }}>
                  <div className="row" style={{ justifyContent: "flex-end", gap: 8 }}>
                    <button onClick={() => openEditModal(p)} className="btnSecondary" type="button">
                      Edit
                    </button>
      ...
      <div className="mobileOnly" style={{ marginTop: 14 }}>
        {filteredProducts.map((p) => (
          <div key={p.id} className="pCard">
            ...
            <div className="pCardMeta">
              {(p.vendor_name ?? vendorNameById.get(p.vendor_id ?? -1) ?? "No vendor")}
              {" • "}
              {(p.material_type_name ?? materialNameById.get(p.material_type_id ?? -1) ?? "Uncategorized")}
            </div>
        ...
        {filteredProducts.length === 0 && <div className="muted">No matching products.</div>}
      </div>

      {/* Add/Edit modal */}
      {modalOpen && (
        <div className="modalOverlay" role="dialog" aria-modal="true" aria-label={editing ? "Edit product" : "Add product"}>
          <div className="modalCard">
            <div className="modalHeader">
              <div className="modalTitle">{editing ? "Edit product" : "Add product"}</div>
              <button className="iconBtn" type="button" onClick={closeModal} aria-label="Close">
                ✕
              </button>
            </div>
            ...
            <div className="formGrid" style={{ marginTop: 10 }}>
              ...
              <div className="locationPicker">
                ...
                {fieldErrors.locations && <div className="fieldError">{fieldErrors.locations}</div>}
              </div>

              <div className="row" style={{ justifyContent: "flex-end", gap: 8, flexWrap: "wrap" }}>
                <button className="btn" type="button" onClick={() => void submitModal()}>
                  Save
                </button>
                <button className="btnSecondary" type="button" onClick={closeModal}>
                  Cancel
                </button>
              </div>
            </div>
...
const styles = `
  :root {
    /* FASTSIGNS brand colors (sampled from logo asset) */
    --fs-red: #C81028;
    --fs-blue: #0030A0;
    --red: var(--fs-red);
    --blue: var(--fs-blue);
  ...
  * { box-sizing: border-box; }
  ...
  .groupHeader { padding: 10px 12px; font-weight: 800; background: var(--blue); color: #ffffff; }
  ...
  .listRowAction { margin-left: auto; }
  ...
  .bottomNavBtn.blue { background: rgba(0, 48, 160, 0.08); border-color: rgba(0, 48, 160, 0.32); color: var(--blue); }
  .bottomNavBtn.red { background: rgba(200, 16, 40, 0.08); border-color: rgba(200, 16, 40, 0.32); color: var(--red); }
  .bottomNavBtn.slate { background: #f8fafc; border-color: #cbd5e1; }

  .bottomNavBtn.active { outline: 3px solid rgba(0,48,160,0.20); }
  ...
  .modalTitle { font-weight: 800; font-size: 18px; color: var(--blue); }
