...
type SettingsTab = "products" | "vendors" | "locations" | "materialTypes" | "import";
...
function Reorder({ onError }: { onError: (m: string) => void }) {
  ...
  const vendorGroups = useMemo(() => {
    ...
    for (const r of rows) {
      const v = (r.vendor_name ?? "No vendor").trim() || "No vendor";
      if (!byVendor.has(v)) byVendor.set(v, []);
      byVendor.get(v)!.push(r);
    }

    const vendorEntries = Array.from(byVendor.entries()).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));

    return vendorEntries.map(([vendorName, vendorRows]) => {
      const byType = new Map<string, ReorderRow[]>();
      for (const r of vendorRows) {
        const t = (r.material_type_name ?? "Uncategorized").trim() || "Uncategorized";
        if (!byType.has(t)) byType.set(t, []);
        byType.get(t)!.push(r);
      }
      const typeEntries = Array.from(byType.entries()).sort((a, b) => a[0].toLowerCase().localeCompare(b[0].toLowerCase()));
      for (const [, list] of typeEntries) list.sort((a, b) => b.to_order - a.to_order);
      return { vendorName, groups: typeEntries };
    });
  }, [rows]);
  ...
  function buildEmailBody() {
    ...
    for (const v of vendorGroups) {
      ...
      for (const [typeName, list] of v.groups) {
        lines.push(`  ${typeName}`);
        for (const r of list) {
...
").trim();
  ...
  return (
    ...
    >
      <div className="hint">Only items that need ordering are shown, grouped by vendor and material category.</div>
      ...
      <div className="noPrint">
        {rows.length === 0 ? (
          ...
          <div className="vendorStack">
            {vendorGroups.map((v) => (
              <div key={v.vendorName} className="vendorBlock">
                <div className="vendorHeader">{v.vendorName}</div>

                <div className="groupStack">
                  {v.groups.map(([groupName, list]) => (
        ...
        <div style={{ marginTop: 12, display: "grid", gap: 16 }}>
          {vendorGroups.map((v) => (
            <div key={v.vendorName}>
              <div style={{ fontWeight: 900, marginBottom: 8, fontSize: 14 }}>{v.vendorName}</div>
...
function VendorsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  return (
    <div>
      <div className="row" style={{ gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="New vendor name" className="input" />
        <button onClick={() => void add()} className="btn" type="button">
          Add
        </button>
      </div>
      ...
      <ul style={{ margin: 0, paddingLeft: 18 }}>
        {items.map((v) => (
          <li key={v.id} style={{ marginBottom: 6 }}>
            {v.name}{" "}
            <button onClick={() => void del(v.id)} className="linkBtn" type="button">
              Delete
            </button>
          </li>
        ))}
        {items.length === 0 && <li>No vendors yet.</li>}
      </ul>
    </div>
  );
}
...
function ImportSettings({ onError }: { onError: (m: string) => void }) {
  ...
  function mapRow(rec: Record<string, string>) {
    ...
    const vendor = pick("vendor", "vendor_name", "supplier");
    ...
    >
      <div className="stack">
        <div className="muted">
          ...
          <div><b>Optional:</b> vendor, material_type, par, location, on_hand</div>
        ...
        {lastResult && (
          <div className="cardSub">
            ...
            <div className="mutedSmall" style={{ marginTop: 6 }}>
              ...
              Locations seen: {lastResult.locations_seen} • Material Types seen: {lastResult.material_types_seen} •
...
function ProductsSettings({ onError }: { onError: (m: string) => void }) {
  ...
  // Keep empty by default so the placeholder label "PAR" is visible like SKU/Product.
  ...
  async function load() {
    try {
      const [prods, mts, vens, locs] = await Promise.all([Api.listProducts(), Api.listMaterialTypes(), Api.listVendors(), Api.listLocations()]);
      setVendors(vens);
      setProducts(prods);
  ...
  async function submit() {
    const n = name.trim();
    const s = sku.trim();
    const p = Math.max(0, round1(Number(par || 0)));
    // These are required per your workflow.
    if (material_type_id === "") {
      onError("Material type is required for a product");
      return;
    }
    if (vendor_id === "") {
      onError("Vendor is required for a product");
      return;
    }
    const mtId = material_type_id;
    const vId = vendor_id;
    ...
    <div>
      ...
      <div className="row" style={{ justifyContent: "flex-end", marginBottom: 10, flexWrap: "wrap", gap: 8 }}>
        <button
          ...
          type="button"
          onClick={() => {
            if (!confirm("Clear ALL products? This cannot be undone. This will also remove assigned locations and on-hand entries.")) return;
      ...
      <div className="mobileOnly" style={{ marginTop: 14 }}>
        {products.map((p) => (
          <div key={p.id} className="pCard">
            ...
            <div className="pCardMeta">{(p.vendor_name ? p.vendor_name + " • " : "") + (p.material_type_name ?? "Uncategorized")}</div>
...
const styles = `
  ...
  * { box-sizing: border-box; }
  ...
  .vendorBlock { border: 1px solid var(--border); border-radius: 16px; overflow: hidden; }
  .vendorHeader { padding: 12px 12px; font-weight: 900; background: #0f172a; color: #ffffff; }
