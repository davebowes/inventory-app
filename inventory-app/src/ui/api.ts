...
export const Api = {
  ...
  createProduct: (p: { name: string; sku: string; material_type_id: ID | null; vendor_id: ID | null; par: number; location_ids: ID[] }) =>
    api<Product>(`/products`, { method: "POST", body: JSON.stringify(p) }),
  updateProduct: (id: ID, p: { name: string; sku: string; material_type_id: ID | null; vendor_id: ID | null; par: number; location_ids: ID[] }) =>
  ...
  importRows: (rows: Array<{ location?: string; material_type?: string; vendor?: string; sku: string; name: string; par?: number | string; on_hand?: number | string }>) =>
    api<{ ok: true; rows_received: number; rows_imported: number; locations_seen: number; material_types_seen: number; vendors_seen: number; on_hand_upserts: number }>(
      `/import`,
