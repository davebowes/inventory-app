      ...
      if (method === "POST") {
        ...
        try {
          const created = await env.DB
            .prepare(
              "INSERT INTO products (name, sku, material_type_id, vendor_id, par) VALUES (?, ?, ?, ?, ?) RETURNING id, name, sku, material_type_id, vendor_id, par"
            )
      ...
      if ((method === "PUT" || method === "PATCH") && parts[1]) {
        ...
        try {
          const r = await env.DB
            .prepare(
              "UPDATE products SET name = ?, sku = ?, material_type_id = ?, vendor_id = ?, par = ? WHERE id = ? RETURNING id, name, sku, material_type_id, vendor_id, par"
            )
