...
async function ensureSchema(db: D1Database) {
  ...
  try {
    await db.prepare("ALTER TABLE products ADD COLUMN notes TEXT").run();
  } catch {
  ...
  try {
    ...
    if (resource === "locations") {
      ...
      if (method === "DELETE" && parts[1]) {
        ...
        const inUse = await env.DB.prepare(
          "SELECT COUNT(1) AS c FROM product_locations WHERE location_id = ?"
        ).bind(id).first<{ c: number }>();
        if ((inUse?.c ?? 0) > 0) {
          return err("Cannot delete this location because one or more products are assigned to it.", 409);
        }
    ...
    if (resource === "material-types") {
      ...
      if (method === "DELETE" && parts[1]) {
        ...
        const inUse = await env.DB.prepare(
          "SELECT COUNT(1) AS c FROM products WHERE material_type_id = ?"
        ).bind(id).first<{ c: number }>();
        if ((inUse?.c ?? 0) > 0) {
          return err("Cannot delete this material type because one or more products are assigned to it.", 409);
        }
    ...
    if (resource === "vendors") {
      ...
      if (method === "DELETE") {
        ...
        const inUse = await env.DB.prepare(
          "SELECT COUNT(1) AS c FROM products WHERE vendor_id = ?"
        ).bind(id).first<{ c: number }>();
        if ((inUse?.c ?? 0) > 0) {
          return err("Cannot delete this vendor because one or more products are assigned to it.", 409);
        }
    ...
    // -------------------
    // POST /api/import
    // Body: { rows: Array<{ location?: string; material_type?: string; vendor?: string; sku: string; name: string; par?: number|string; on_hand?: number|string }> }
    if (resource === "import") {
      ...
      const body = await readJson<{ rows?: ImportRow[]; dry_run?: boolean; dedup_mode?: "update" | "skip" }>(request);
      const rows = Array.isArray(body.rows) ? body.rows : [];
      if (rows.length === 0) return err("rows[] is required");

      const dryRun = Boolean(body.dry_run);
      const dedupMode: "update" | "skip" = body.dedup_mode === "skip" ? "skip" : "update";
      ...
      // Pull existing reference names for preview/warnings.
      const existingLocs = await env.DB.prepare("SELECT name FROM locations").all();
      const existingMts = await env.DB.prepare("SELECT name FROM material_types").all();
      const existingVendors = await env.DB.prepare("SELECT name FROM vendors").all();
      const locSet = new Set((existingLocs.results ?? []).map((r: any) => String(r.name)));
      const mtSet = new Set((existingMts.results ?? []).map((r: any) => String(r.name)));
      const vendorSet = new Set((existingVendors.results ?? []).map((r: any) => String(r.name)));
      const newLocations = locNames.filter((n) => !locSet.has(n));
      const newMaterialTypes = mtNames.filter((n) => !mtSet.has(n));
      const newVendors = vendorNames.filter((n) => !vendorSet.has(n));
      ...
      // Determine existing SKUs (for preview + skip mode)
      const allSkus = Array.from(new Set(normRows.map((r) => r.sku)));
      const existingSkusSet = new Set<string>();
      if (allSkus.length) {
        const rs = await env.DB
          .prepare("SELECT sku FROM products WHERE sku IN (" + allSkus.map(() => "?").join(",") + ")")
          .bind(...allSkus)
          .all();
        for (const row of (rs.results ?? []) as any[]) existingSkusSet.add(String(row.sku));
      }

      const willInsert = allSkus.filter((s) => !existingSkusSet.has(s));
      const willUpdate = allSkus.filter((s) => existingSkusSet.has(s));
      ...
      if (dryRun) {
        return json({
          ...
          new_material_types: newMaterialTypes,
          new_vendors: newVendors,
          existing_skus: Array.from(existingSkusSet),
          will_insert_products: willInsert.length,
          will_update_products: dedupMode === "update" ? willUpdate.length : 0,
          will_skip_product_updates: dedupMode === "skip" ? willUpdate.length : 0,
          on_hand_rows: normRows.filter((r) => r.location && r.on_hand !== undefined && r.on_hand !== null && r.on_hand !== "").length,
        });
      }

      // Upsert/insert products by sku
      const productStmts = normRows.map((r) => {
        ...
        if (dedupMode === "skip") {
          return env.DB
            .prepare(
              `INSERT INTO products (name, sku, material_type_id, vendor_id, par, notes)
               VALUES (?, ?, ?, ?, ?, ?)
               ON CONFLICT(sku) DO NOTHING`
            )
            .bind(r.name, r.sku, mtId, vendorId, par, notes || null);
        }
        return env.DB
          .prepare(
            `INSERT INTO products (name, sku, material_type_id, vendor_id, par, notes)
      ...
      return json({
        ...
        products_inserted: willInsert.length,
        products_updated: dedupMode === "update" ? willUpdate.length : 0,
        product_updates_skipped: dedupMode === "skip" ? willUpdate.length : 0,
    ...
    if (resource === "products") {
      if (method === "GET") {
        const [prods, pls] = await Promise.all([
          env.DB
            .prepare(
              `SELECT p.id, p.name, p.sku, p.material_type_id, mt.name AS material_type_name, p.vendor_id, v.name AS vendor_name, p.par, p.notes
               FROM products p
      ...
      if (method === "POST") {
        ...
        try {
          const created = await env.DB
            .prepare(
              "INSERT INTO products (name, sku, material_type_id, vendor_id, par, notes) VALUES (?, ?, ?, ?, ?, ?) RETURNING id, name, sku, material_type_id, vendor_id, par, notes"
            )
            .bind(name, sku, material_type_id, vendor_id, par, notes)
            .first<any>();
      ...
      if ((method === "PUT" || method === "PATCH") && parts[1]) {
        ...
        try {
          const r = await env.DB
            .prepare(
              "UPDATE products SET name = ?, sku = ?, material_type_id = ?, vendor_id = ?, par = ?, notes = ? WHERE id = ? RETURNING id, name, sku, material_type_id, vendor_id, par, notes"
            )
            .bind(name, sku, material_type_id, vendor_id, par, notes, id)
            .first<any>();
